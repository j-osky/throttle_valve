...
#define PT_SENSOR_PGN       0x00FF50u   /* Combined PT sensor broadcast PGN.   */
#define PT_SENSOR_SA        0xA5u       /* Source address of external PT board.*/
#define PT_SENSOR_SCALE     10.0        /* Payload stores psi * 10 as uint16.  */
#define PT_SENSOR_EID_BASE  0x18FF5000u /* Priority 6, PDU2 PGN 0x00FF50.      */
...
typedef struct {
    ...
    /* Sensor readings (from PT CAN board) */
    double      pom_psi;
    double      pfm_psi;
    double      pc_psi;
...
static uint16_t u16_from_le(const uint8_t *p)
{
    return (uint16_t)p[0] | ((uint16_t)p[1] << 8);
}

static bool is_pt_sensor_frame(const struct can_frame *f)
{
    uint32_t id = f->can_id & CAN_EFF_MASK;
    return (f->can_id & CAN_EFF_FLAG) &&
           id == (PT_SENSOR_EID_BASE | PT_SENSOR_SA) &&
           f->can_dlc >= 6;
}

static bool sensor_cache_update_from_can(const struct can_frame *f)
{
    if (!is_pt_sensor_frame(f)) return false;

    pthread_mutex_lock(&sensor_mutex);
    g_sensors.pom_psi = (double)u16_from_le(&f->data[0]) / PT_SENSOR_SCALE;
    g_sensors.pfm_psi = (double)u16_from_le(&f->data[2]) / PT_SENSOR_SCALE;
    g_sensors.pc_psi  = (double)u16_from_le(&f->data[4]) / PT_SENSOR_SCALE;
                           ...
                           size_t filter_count)
{
    ...
    setsockopt(fd, SOL_CAN_RAW, CAN_RAW_ERR_FILTER, &em, sizeof(em));

    if (filters && filter_count > 0) {
        if (setsockopt(fd, SOL_CAN_RAW, CAN_RAW_FILTER,
                       filters, filter_count * sizeof(filters[0])) < 0) {
...
{
    can_sock = can_open_socket(iface, NULL, 0);
    if (can_sock < 0) return -1;

    const struct can_filter sensor_filter = {
        .can_id   = (PT_SENSOR_EID_BASE | PT_SENSOR_SA) | CAN_EFF_FLAG,
        .can_mask = CAN_EFF_FLAG | CAN_EFF_MASK,
    };
    sensor_sock = can_open_socket(iface, &sensor_filter, 1);
    if (sensor_sock < 0) {
        close(can_sock);
        can_sock = -1;
        return -1;
    ...
    printf("[CAN] Opened %s (main fd=%d, sensor fd=%d)\n",
           iface, can_sock, sensor_sock);
    return 0;
}
...
static void can_process_rx(void)
{
    ...
    while (read(can_sock, &f, sizeof(f)) == sizeof(f)) {
        ...
        if (sensor_cache_update_from_can(&f)) continue;
...
/* ══════════════════════════════════════════════════════════════════════════
 * PT sensor CAN receive path
 * ══════════════════════════════════════════════════════════════════════════ */

/* ══════════════════════════════════════════════════════════════════════════
 * PT sensor background thread
 * Reads combined PT frames from CAN and writes to g_sensors cache.
 * The control loop reads from g_sensors without blocking (<1 us mutex read).
 * If no fresh frame arrives, the previous sample is held until the timeout
 * watchdog trips during RUNNING.
 * ══════════════════════════════════════════════════════════════════════════ */
static void *sensor_thread(void *arg)
{
    ...
    while (g_running) {
        ssize_t n = read(sensor_sock, &f, sizeof(f));
        if (n == sizeof(f)) {
            (void)sensor_cache_update_from_can(&f);
            continue;
        }
