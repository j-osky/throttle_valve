...
static void can_process_rx(void)
{
    ...
    while (read(can_sock, &f, sizeof(f)) == sizeof(f)) {
        ...
        if ((pgn & 0x1FF00u) == 0x0EF00u && is_known_valve && f.can_dlc == 8) {
            ...
            if (sa == KZVALVE_SA_LOX)
                g_valve_angles.lox_deg = pos_deg;
            else if (sa == KZVALVE_SA_IPA)
                g_valve_angles.ipa_deg = pos_deg;
 ...
 *
 * When real valves are connected (no mock_can_bridge), mock_daq has no way
 * to know actual valve positions unless tv_main tells it.  This thread reads
 * g_valve_angles (written by can_process_rx from real CAN feedback) and
 * POSTs to mock_daq /mock/valve_angles at 10 Hz so the mock physics reflects
 * what the real valves are doing and PT readings respond accordingly.
 *
 * If mock_daq is not running the POST fails silently — no impact on control.
 * ══════════════════════════════════════════════════════════════════════════ */
static void *daq_push_thread(void *arg)
{
    ...
    curl_easy_setopt(curl, CURLOPT_TIMEOUT_MS, 80L);    /* 80ms — non-blocking enough */
    curl_easy_setopt(curl, CURLOPT_NOSIGNAL,   1L);
    curl_easy_setopt(curl, CURLOPT_POST,       1L);

    struct timespec next;
    clock_gettime(CLOCK_MONOTONIC, &next);
    const long PUSH_PERIOD_NS = (1000000000L / 10);   /* 100ms = 10 Hz */
    ...
    while (g_running) {
        ...
        curl_easy_perform(curl);   /* fire-and-forget; ignore errors */
